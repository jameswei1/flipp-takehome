...
ssh -i flipp-key -o StrictHostKeychecking=no ec2-user@$(terraform output -raw public_ip) "sudo yum install nodejs -y;cd flipp-takehome;npm install;node server.js"
